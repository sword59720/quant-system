...
def main():
    ...
    try:
        ...
        def calculate_boll(price, volume, window=20, num_std=2):
            ...
            # 成交量放大
            volume_expand = (volume_change > 0.3).astype(int)
            
            # 计算信号强度
            # 信号1：价格突破下轨 + 成交量萎缩 + 布林带开口向上（超跌反弹）
            buy_signal1_strength = lower_break * volume_shrink * boll_open_up * (1 + abs(volume_change))
            # 信号1：价格突破上轨 + 成交量萎缩 + 布林带开口向下（超买回调）
            sell_signal1_strength = upper_break * volume_shrink * boll_open_down * (1 + abs(volume_change))
            # 信号2：价格突破上轨 + 成交量放大 + 布林带开口向上（趋势确认）
            buy_signal2_strength = upper_break * volume_expand * boll_open_up * (1 + volume_change)
            # 信号2：价格跌破下轨 + 成交量放大 + 布林带开口向下（趋势确认）
            sell_signal2_strength = lower_break * volume_expand * boll_open_down * (1 + volume_change)
            
            # 组合信号强度，给予不同权重
            buy_signal_strength = buy_signal1_strength * 1.5 + buy_signal2_strength
            sell_signal_strength = sell_signal1_strength * 1.5 + sell_signal2_strength
            
            # 根据信号强度生成最终信号
            buy_signal = (buy_signal_strength > 0.5).astype(int)
            sell_signal = (sell_signal_strength > 0.5).astype(int)
