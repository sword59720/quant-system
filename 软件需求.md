...
- `scripts/run_crypto.py`：币圈信号引擎（多因子 + risk-off + 合约模式防守/可选做空）。
...
- `scripts/validate_stock_cpcv.py`：CPCV 稳健性验证（分组组合切片 + 门槛判定 + N/k/embargo 网格搜索）。
- `scripts/backtest_v3.py`：股票/币圈轻量双市场回测。
- `scripts/validate_crypto_contract.py`：币圈合约模型稳健性验证（全样本/OOS/成本压力）。
- `scripts/optimize_crypto_contract.py`：币圈合约参数随机搜索（兼顾 full/OOS/stress 目标）。
...
- `python3 scripts/validate_crypto_contract.py`
- `python3 scripts/optimize_crypto_contract.py --trials 50 --seed 42`
...
- 已固化基线快照：`stock_prod_baseline_20260220_125141`（最新对照组，沿用 `outputs/reports/baseline_latest.json` 指针管理）。
...
8. 通过 `generate_trades.py` 与当前持仓做差，输出 `BUY/SELL` 拟交易清单（`stock_trades.json`）。
...
- 第四步：选择满足“年化不降且回撤更优”的参数组合落地，并重跑 `backtest_v3.py` 校验。
