...
from scripts.crypto.execute_trades_crypto import build_crypto_order
from scripts.stock_etf.execute_trades_stock_etf import build_stock_order
...
class TestExecuteTradesOrderBuild(unittest.TestCase):
    def test_build_stock_order(self):
        order = build_stock_order({"symbol": "510300", "action": "BUY", "amount_quote": 1000})
        self.assertEqual(order.side, StockOrderSide.BUY)
        self.assertEqual(order.amount, 1000)

    def test_build_crypto_order_basic(self):
        order = build_crypto_order({"symbol": "BTC/USDT", "action": "SELL", "amount_quote": 500})
    ...
    def test_build_crypto_order_close_short(self):
        order = build_crypto_order({"symbol": "BTC/USDT", "action": "CLOSE_SHORT", "amount_quote": 200})
    ...
    def test_build_crypto_order_invalid_action(self):
        with self.assertRaises(ValueError):
            build_crypto_order({"symbol": "BTC/USDT", "action": "UNKNOWN", "amount_quote": 100})
