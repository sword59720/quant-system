...
def first_matching_col(columns: List[str], keywords: List[str]) -> Optional[str]:
    normalized_cols = [str(c) for c in columns]

    # Prefer exact match to avoid collisions such as:
    # "大单净流入-净额" being matched by "超大单净流入-净额".
    lower_to_col = {c.lower(): c for c in normalized_cols}
    for kw in keywords:
        k = str(kw).lower()
...
def normalize_valuation_frame(df: pd.DataFrame, start_date: str) -> pd.DataFrame:
    out_cols = ["date", "pe_ttm", "pb", "ps_ttm", "pcf_ncf_ttm"]
    if df is None or df.empty:
    ...
    for c in ["pe_ttm", "pb", "ps_ttm", "pcf_ncf_ttm"]:
        if c not in x.columns:
            x[c] = float("nan")
        else:
            x[c] = pd.to_numeric(x[c], errors="coerce")

    # Zero/negative valuation values are treated as invalid.
    x.loc[x["pe_ttm"] <= 0, "pe_ttm"] = float("nan")
    x.loc[x["pb"] <= 0, "pb"] = float("nan")

    start_dt = pd.to_datetime(start_date, errors="coerce")
    if pd.notna(start_dt):
        x = x[x["date"] >= start_dt].reset_index(drop=True)

    x = x.drop_duplicates(subset=["date"], keep="last").sort_values("date").reset_index(drop=True)
    x = x.dropna(subset=["pe_ttm", "pb"], how="all")
    if x.empty:
        return pd.DataFrame(columns=out_cols)
    return x[out_cols].copy()


def valuation_quality_score(df: pd.DataFrame) -> tuple:
    if df is None or df.empty:
        return (0, 0, 0, 0, 0)
    valid_pe = int(df["pe_ttm"].notna().sum()) if "pe_ttm" in df.columns else 0
    valid_pb = int(df["pb"].notna().sum()) if "pb" in df.columns else 0
    rows = int(len(df))
    total_valid = valid_pe + valid_pb
    history_rich = 1 if rows >= 60 else 0
    return (history_rich, total_valid, rows, valid_pe, valid_pb)
...
def fetch_valuation_with_multiple_sources(symbol_code: str, start_date: str) -> pd.DataFrame:
    ...
    best_source = ""
    best_df = pd.DataFrame(columns=["date", "pe_ttm", "pb", "ps_ttm", "pcf_ncf_ttm"])
    best_quality = (0, 0, 0, 0, 0)

    for source_name, fetch_func in sources:
        try:
            print(f"[stock-single-data][valuation] Trying {source_name} for {symbol_code}")
            raw_df = fetch_func()
            df = normalize_valuation_frame(raw_df, start_date=start_date)

            if df.empty:
                print(f"[stock-single-data][valuation] {source_name} returned empty data for {symbol_code}")
                continue

            score = valuation_quality_score(df)
            data_quality = {
                "length": len(df),
            ...
            print(f"[stock-single-data][valuation] Candidate {source_name} for {symbol_code} with {len(df)} rows")
    ...
    if not best_df.empty:
        print(
            f"[stock-single-data][valuation] Selected best source {best_source} for {symbol_code} "
            f"with quality={list(best_quality)} rows={len(best_df)}"
        )
        return best_df
...
def normalize_fund_flow_frame(df: pd.DataFrame, start_date: str) -> pd.DataFrame:
    out_cols = [
        "date",
        "main_net_inflow",
    ...
    x = x.drop_duplicates(subset=["date"], keep="last").sort_values("date").reset_index(drop=True)
    x = x.dropna(subset=["main_net_inflow", "main_net_inflow_ratio"], how="all")
    if x.empty:
        return pd.DataFrame(columns=out_cols)

    extra_cols = []
    if "flow_data_source" in x.columns:
        extra_cols.append("flow_data_source")
    return x[out_cols + extra_cols].copy()


def fund_flow_quality_score(df: pd.DataFrame) -> tuple:
    if df is None or df.empty:
        return (0, 0, 0, 0, 0)
    valid_main = int(df["main_net_inflow"].notna().sum()) if "main_net_inflow" in df.columns else 0
    valid_ratio = int(df["main_net_inflow_ratio"].notna().sum()) if "main_net_inflow_ratio" in df.columns else 0
    rows = int(len(df))
    total_valid = valid_main + valid_ratio
    history_rich = 1 if rows >= 60 else 0
    return (history_rich, total_valid, rows, valid_main, valid_ratio)


def build_effective_fund_flow_frame(real_df: pd.DataFrame, proxy_df: pd.DataFrame, merge_mode: str) -> pd.DataFrame:
    mode = str(merge_mode).strip().lower()
    if mode not in {"real_only", "real_with_proxy_backfill", "proxy_only"}:
        mode = "real_only"
    ...
    # real_with_proxy_backfill: use proxy for history; overwrite same-date rows with real values.
    if proxy_df.empty:
        return real_df.copy()
    if real_df.empty:
        return proxy_df.copy()
    return merge_history(proxy_df, real_df, time_col="date")
...
def fetch_fund_flow_with_multiple_sources(
    symbol_code: str,
    canonical: str,
    start_date: str,
    ...
    """
    # Real sources first. Proxy source can be included explicitly.
    sources = [
        ...
        ("sina", lambda: fetch_fund_flow_with_sina(symbol_code, canonical, start_date)),
    ]
    if include_proxy_source:
        sources.append(("baostock_proxy", lambda: fetch_fund_flow_with_baostock(symbol_code, canonical, start_date)))
    ...
    for source_name, fetch_func in sources:
        try:
            ...
            raw_df = fetch_func()
            df = normalize_fund_flow_frame(raw_df, start_date=start_date)
            if df.empty:
                print(f"[stock-single-data][flow] {source_name} returned empty data for {canonical}")
                continue

            df = df.copy()
            df["flow_data_source"] = source_name
            score = fund_flow_quality_score(df)
            data_quality = {
                "length": int(len(df)),
                "valid_main_inflow": int(df["main_net_inflow"].notna().sum()),
                "valid_main_ratio": int(df["main_net_inflow_ratio"].notna().sum()),
                "total_valid": int(df[["main_net_inflow", "main_net_inflow_ratio"]].notna().sum().sum()),
                "quality_score": list(score),
            }

            print(f"[stock-single-data][flow] Candidate {source_name} for {canonical} with {len(df)} rows")
    ...
    if not best_df.empty:
        print(
            f"[stock-single-data][flow] Selected best source {best_source} for {canonical} "
            f"with quality={list(best_quality)} rows={len(best_df)}"
        )
        return best_df
...
def main():
    ...
    flow_merge_mode = str(data_cfg.get("fund_flow_merge_mode", "real_only")).strip().lower()
    if flow_merge_mode not in {"real_only", "real_with_proxy_backfill", "proxy_only"}:
        flow_merge_mode = "real_only"

    flow_real_dir = str(data_cfg.get("fund_flow_real_output_dir", flow_dir))
    flow_proxy_dir = str(data_cfg.get("fund_flow_proxy_output_dir", os.path.join(base_data_dir, "stock_single", "fund_flow_proxy")))
    if (flow_merge_mode != "real_only") and (os.path.abspath(flow_real_dir) == os.path.abspath(flow_dir)):
        flow_real_dir = os.path.join(base_data_dir, "stock_single", "fund_flow_real")
    flow_proxy_enabled = bool(data_cfg.get("fund_flow_proxy_enabled", flow_merge_mode != "real_only"))
    ...
    ensure_dir(flow_dir)
    ensure_dir(flow_real_dir)
    if flow_proxy_enabled:
        ensure_dir(flow_proxy_dir)
    ...
    print(f"[stock-single-data] - Fund flow (merged): {flow_dir}")
    print(f"[stock-single-data] - Fund flow (real): {flow_real_dir}")
    print(f"[stock-single-data] - Fund flow (proxy): {flow_proxy_dir} (enabled={flow_proxy_enabled})")
    print(f"[stock-single-data] - Fund flow merge mode: {flow_merge_mode}")
    ...
    if fetch_valuation:
        ...
        hist_years_valuation = int(data_cfg.get("history_years_valuation", 5))
        default_start_valuation = now - timedelta(days=365 * hist_years_valuation)
        ...
        print(f"[stock-single-data][valuation] Default start date: {default_start_valuation.strftime('%Y-%m-%d')}")
        ...
        for i, (code, canonical) in enumerate(symbols, 1):
            ...
            try:
                if has_valid_cache(
                    ...
                    start_dt = resolve_incremental_start(
                        out_file,
                        default_start=default_start_valuation,
                        overlap_days=overlap_days,
                        time_col="date",
                    )
                    fetch_mode = "incremental"
                else:
                    start_dt = default_start_valuation
                    fetch_mode = "full_backfill"
                start_str = start_dt.strftime("%Y-%m-%d")
                print(f"[stock-single-data][valuation] Fetching from {start_str} ({fetch_mode})")
                ...
                df_merge["date"] = pd.to_datetime(df_merge["date"]).dt.strftime("%Y-%m-%d")
                df_merge.to_csv(out_file, index=False, encoding="utf-8")
                if len(df_merge) < min_rows_valuation:
                    print(
                        f"[stock-single-data][valuation] warning: {canonical} rows={len(df_merge)} "
                        f"< min_rows_valuation={min_rows_valuation}"
                    )
    ...
    if fetch_fund_flow:
        ...
        hist_years_flow_proxy = int(
            data_cfg.get(
                "history_years_fund_flow_proxy",
                max(hist_years_flow, int(data_cfg.get("history_years_daily", 5))),
            )
        )
        default_start_flow_real = now - timedelta(days=365 * hist_years_flow)
        default_start_flow_proxy = now - timedelta(days=365 * hist_years_flow_proxy)
        ...
        min_rows_flow_proxy = int(guard_cfg.get("min_cache_rows_fund_flow_proxy", max(240, min_rows_flow)))
        max_age_flow_proxy = int(guard_cfg.get("max_cache_age_days_fund_flow_proxy", max(30, max_age_flow)))
        use_proxy_for_merge = bool(flow_proxy_enabled and flow_merge_mode in {"real_with_proxy_backfill", "proxy_only"})
        effective_start_flow = min(default_start_flow_real, default_start_flow_proxy) if use_proxy_for_merge else default_start_flow_real
        ...
        for i, (code, canonical) in enumerate(symbols, 1):
            ...
            real_out_file = os.path.join(flow_real_dir, f"{canonical}.csv")
            proxy_out_file = os.path.join(flow_proxy_dir, f"{canonical}.csv")
            try:
                if has_valid_cache(
                    real_out_file,
                    time_col="date",
                    ...
                    start_real_dt = resolve_incremental_start(
                        real_out_file,
                        default_start=default_start_flow_real,
                        overlap_days=overlap_days,
                        time_col="date",
                    )
                    real_fetch_mode = "incremental"
                else:
                    start_real_dt = default_start_flow_real
                    real_fetch_mode = "full_backfill"
                start_real_str = start_real_dt.strftime("%Y-%m-%d")
                print(f"[stock-single-data][flow] Real fetch from {start_real_str} ({real_fetch_mode})")

                real_old = normalize_fund_flow_frame(
                    read_existing(real_out_file, time_col="date"),
                    start_date=default_start_flow_real.strftime("%Y-%m-%d"),
                )
                real_new = fetch_fund_flow_with_multiple_sources(
                    symbol_code=code,
                    canonical=canonical,
                    start_date=start_real_str,
                    include_proxy_source=False,
                )
                real_new = normalize_fund_flow_frame(real_new, start_date=start_real_str)
                if not real_new.empty:
                    real_new = real_new.copy()
                    real_new["flow_data_source"] = real_new.get("flow_data_source", "real")
                real_merge = normalize_fund_flow_frame(
                    merge_history(real_old, real_new, time_col="date"),
                    start_date=default_start_flow_real.strftime("%Y-%m-%d"),
                )

                if not real_merge.empty:
                    if "flow_data_source" not in real_merge.columns:
                        real_merge["flow_data_source"] = "real"
                    x_real = real_merge.copy()
                    x_real["date"] = pd.to_datetime(x_real["date"]).dt.strftime("%Y-%m-%d")
                    x_real.to_csv(real_out_file, index=False, encoding="utf-8")

                proxy_merge = pd.DataFrame()
                if use_proxy_for_merge:
                    if has_valid_cache(
                        proxy_out_file,
                        time_col="date",
                        min_rows=min_rows_flow_proxy,
                        max_age_days=max_age_flow_proxy,
                    ):
                        start_proxy_dt = resolve_incremental_start(
                            proxy_out_file,
                            default_start=default_start_flow_proxy,
                            overlap_days=overlap_days,
                            time_col="date",
                        )
                        proxy_fetch_mode = "incremental"
                    else:
                        start_proxy_dt = default_start_flow_proxy
                        proxy_fetch_mode = "full_backfill"
                    start_proxy_str = start_proxy_dt.strftime("%Y-%m-%d")
                    print(f"[stock-single-data][flow] Proxy fetch from {start_proxy_str} ({proxy_fetch_mode})")

                    proxy_old = normalize_fund_flow_frame(
                        read_existing(proxy_out_file, time_col="date"),
                        start_date=default_start_flow_proxy.strftime("%Y-%m-%d"),
                    )
                    proxy_new = fetch_fund_flow_with_baostock(symbol_code=code, canonical=canonical, start_date=start_proxy_str)
                    proxy_new = normalize_fund_flow_frame(proxy_new, start_date=start_proxy_str)
                    if not proxy_new.empty:
                        proxy_new = proxy_new.copy()
                        proxy_new["flow_data_source"] = "baostock_proxy"
                    proxy_merge = normalize_fund_flow_frame(
                        merge_history(proxy_old, proxy_new, time_col="date"),
                        start_date=default_start_flow_proxy.strftime("%Y-%m-%d"),
                    )

                    if not proxy_merge.empty:
                        if "flow_data_source" not in proxy_merge.columns:
                            proxy_merge["flow_data_source"] = "baostock_proxy"
                        x_proxy = proxy_merge.copy()
                        x_proxy["date"] = pd.to_datetime(x_proxy["date"]).dt.strftime("%Y-%m-%d")
                        x_proxy.to_csv(proxy_out_file, index=False, encoding="utf-8")

                df_merge = build_effective_fund_flow_frame(real_df=real_merge, proxy_df=proxy_merge, merge_mode=flow_merge_mode)
                df_merge = normalize_fund_flow_frame(df_merge, start_date=effective_start_flow.strftime("%Y-%m-%d"))

                # Validate merged data
                if df_merge.empty:
                    raise RuntimeError("empty merged fund flow data")

                # Process and save effective merged data
                x_merge = df_merge.copy()
                x_merge["date"] = pd.to_datetime(x_merge["date"]).dt.strftime("%Y-%m-%d")
                x_merge.to_csv(out_file, index=False, encoding="utf-8")

                real_rows = int(len(real_merge))
                proxy_rows = int(len(proxy_merge)) if use_proxy_for_merge else 0
                merged_rows = int(len(df_merge))
                print(
                    f"[stock-single-data][flow] Success: {canonical} "
                    f"real={real_rows} proxy={proxy_rows} merged={merged_rows}"
                )
                ok_flow += 1
            except Exception as e:
                if has_valid_cache(
