...
# 导入适配器
from adapters.guotou_trader import (
    create_trader as create_guotou_trader,
    Order as StockOrder,
    OrderSide as StockOrderSide,
)
from adapters.myquant_trader import create_trader as create_myquant_trader
from adapters.crypto_trader import (
    create_trader as create_crypto_trader,
    Order as CryptoOrder,
    OrderSide as CryptoOrderSide,
)
...
def build_order_for_market(market: str, trade: dict):
    symbol = trade.get("symbol")
    action_raw = str(trade.get("action", "BUY")).strip().upper()
    amount = float(trade.get("amount_quote", 0) or 0)

    if market == "crypto":
        # 支持现有 BUY/SELL，以及合约常见动作
        # OPEN/CLOSE 在单向持仓和双向持仓中都可映射到 buy/sell
    ...
    if action_raw not in {"BUY", "SELL"}:
        raise ValueError(f"unsupported stock action: {action_raw}")
    side = StockOrderSide.BUY if action_raw == "BUY" else StockOrderSide.SELL
...
def _reference_price(config: dict, market: str, symbol: str, cache: dict):
    key = f"{market}:{symbol}"
    if key in cache:
        return cache[key]
...
def _calc_slippage_bps(action: str, ref_price, fill_price):
    ...
    # Positive value means worse execution quality.
...
def execute_trades(trades_file: str, dry_run: bool = False):
    ...
    try:
        ...
        for i, trade in enumerate(orders, 1):
            ...
            ref_px = _reference_price(config, market, symbol, ref_price_cache)
            
            try:
                order = build_order_for_market(market, trade)
            except Exception as e:
                failed_item = {
                    "symbol": symbol,
                    "action": action,
                ...
                failed_orders.append(failed_item)
        ...
        # 同步持仓
        pos_file = "./outputs/state/crypto_positions.json" if market == "crypto" else "./outputs/state/stock_positions.json"
        try:
