...
- 关键脚本接入退出码与异常分类：
  - `/Users/haojc/work/quant-system/scripts/stock_etf/fetch_stock_etf_data.py`
...
- 新增测试：
  - `/Users/haojc/work/quant-system/tests/test_stock_etf_production_search.py`
...
### 2. 验证
- 新增测试：
  - `/Users/haojc/work/quant-system/tests/test_backtest_stock_etf_compare.py`
...
### 2. 新增脚本与测试
- 新增脚本：
  - `/Users/haojc/work/quant-system/scripts/stock_etf/backtest_stock_etf_cpcv.py`
- 新增测试：
  - `/Users/haojc/work/quant-system/tests/test_stock_etf_cpcv_validation.py`

### 3. 实跑结果
- 执行：`python3 scripts/stock_etf/backtest_stock_etf_cpcv.py`
...
- `scripts/stock_etf/backtest_stock_etf_cpcv.py` 增强：
...
- CPCV（`backtest_stock_etf_cpcv.py`）：
...
## 2026-02-20 收益防守折中版基线固化（准备实盘运行）

### 1. 基线快照
- Baseline ID：`stock_prod_baseline_20260220_125141`
- 路径：`/Users/haojc/work/quant-system/outputs/reports/baselines/stock_prod_baseline_20260220_125141`
- 快照索引：`/Users/haojc/work/quant-system/outputs/reports/baseline_latest.json`
- Manifest：`/Users/haojc/work/quant-system/outputs/reports/baselines/stock_prod_baseline_20260220_125141/baseline_manifest.json`

### 2. 当前基线指标（production / paper-forward 口径）
- 年化收益：`13.0796%`
- 最大回撤：`-11.8355%`
- Sharpe：`1.0975`
- 说明：相较旧参数，年化基本不降，回撤显著收敛。

### 3. 本次固化内容
- 配置：`stock.yaml`、`risk.yaml`、`runtime.yaml`
- 运行输出：`stock_targets.json`
- 回测与验证：`backtest_report.json`、`backtest_compare.json`、`stock_paper_forward_latest.json`、`stock_paper_forward_latest_risk_governor_snapshot.json`、`stock_paper_forward_history.csv`、`stock_cpcv_report.json`
- 搜索证据：`stock_risk_governor_wide_search.json`
- 每个文件已记录 `sha256` 与 `size_bytes`，用于后续回归对照。

## 2026-02-21 年化优先迭代后基线固化（return-defense balance v2）

### 1. 基线快照
- Baseline ID：`stock_prod_baseline_20260221_233115`
- 路径：`/Users/haojc/work/quant-system/outputs/reports/baselines/stock_prod_baseline_20260221_233115`
- 快照索引：`/Users/haojc/work/quant-system/outputs/reports/baseline_latest.json`
- Manifest：`/Users/haojc/work/quant-system/outputs/reports/baselines/stock_prod_baseline_20260221_233115/baseline_manifest.json`

### 2. 当前基线指标（production / paper-forward 口径）
- 年化收益：`15.9469%`
- 最大回撤：`-10.0831%`
- Sharpe：`1.1939`
- 对比上一基线（`13.0796% / -11.8355% / 1.0975`）：
  - 年化 `+2.8673pct`
  - 回撤改善 `+1.7524pct`（绝对回撤收敛）
  - Sharpe `+0.0964`
...
- 运行输出：`stock_targets.json`
- 回测与验证：
  - `backtest_stock_etf_report.json`
  - `backtest_stock_etf_compare.json`
  - `backtest_report.json`
  - `backtest_compare.json`
  - `stock_backtest_snapshot.csv`
  - `stock_backtest_snapshot_meta.json`
  - `stock_paper_forward_latest.json`
  - `stock_paper_forward_latest_risk_governor_snapshot.json`
  - `stock_paper_forward_history.csv`
  - `stock_cpcv_report.json`
- 搜索证据：
  - `stock_target_optimization_report.json`
  - `stock_target_search_20_2_probe.json`
  - `stock_target_search_20_2_local.json`
  - `stock_target_search_20_2_aggressive.json`

### 4. 结论（目标可达性）
- 已执行三轮参数搜索（共 `1052` 组）：
  - `年化 > 20%` 且 `Sharpe > 2` 的全周期组合数量：`0`
  - 当前框架下最优折中为本次基线（收益、回撤、Sharpe 更均衡）。

## 2026-03-16 ETF 池去重 + 债券防守补强 + 513100 数据治理

### 1. 触发原因
- 对当前 ETF 池做相似度/相关性审计后，确认存在明显重复与拥挤：
  - `159915 / 159949`
  - `159633 / 512100`
  - `510500 / 512100 / 159633`
- 同时评估是否加入债券 ETF 作为防护仓候选。
...
- 保留并限仓：
  ...
  - `513100 <= 0.35`
- 新增债券防守候选：`511010`
- 债券上限测试结论：`511010 cap40` 优于 `cap50`
- 当前主配置已切换为：`config/stock.yaml`
- 原配置备份：`config/stock.yaml.bak_20260316_1131`

### 3. 债券候选回测结论
- `511010` 优于 `511260`
- `511010` 在当前框架中并非替代 `518880`，而是与黄金构成双防守层
- 在修复历史脏数据前，`cap50` 曾看似优于 `cap40`；清洗后确认该优势主要来自 `513100` 异常历史数据

### 4. 数据问题定位与修复（`513100`）
- 发现旧文件 `data/stock/513100.csv` 在 2016-12 ~ 2017-03 混入两套价格标尺（约 `0.4x` 与 `2.0x`）
- 异常表现：单日 `+300% ~ +400%` 级别伪收益，显著污染全样本回测
- 处理：
  - 旧文件备份：`data/stock/513100.csv.bak_20260316_mixed_scale`
  - 新文件：改用 Sina 单一来源全量重建

### 5. 清洗后用于决策的主候选结果
来源：`outputs/reports/candidate_compare/bond511010_cap40.clean513100.report.json`
...
### 6. 当前生产候选判断
- 当前更可信的主推进版：`stock_candidate_main_next_cap40_20260316.yaml`
- 已晋升到主配置：`config/stock.yaml`
- 结论：先以 `511010 cap40` 作为主推进方案，后续再决定是否升级为显式 `defensive_candidates` 架构。

## 2026-02-21 结构级升级（Phase 1：三状态 + 复合动量）

### 1. 实现内容
- 在以下文件中接入结构级逻辑（非单纯调参）：
  - `/Users/haojc/work/quant-system/scripts/stock_etf/paper_forward_stock.py`
  - `/Users/haojc/work/quant-system/scripts/stock_etf/run_stock_etf.py`
- 新增能力：
  - `risk_on / neutral / risk_off` 三状态分层（基准趋势 + breadth）
  - 多周期复合动量（`momentum_windows + momentum_weights`）
  - 相对基准动量分量（`relative_momentum_*`）
  - 结构仓位乘子（trend/breadth/total alloc multipliers）
  - 结构信号与原长周期动量混合（`score_blend`）
  - 可配置是否要求资产趋势过滤（`eligibility_require_trend`）

### 2. 搜索与结果
- 新增结构搜索报告：
  - `/Users/haojc/work/quant-system/outputs/reports/stock_structural_search_v1.json`
- 本轮结构搜索规模：`260` 组
- 结果：
  - `年化 > 20%` 命中：`0`
  - `Sharpe > 2` 命中：`0`
  - 双目标同时命中：`0`
  - best_ann：`16.4955% / -21.0816% / 1.0050`
  - best_sh：`12.7096% / -11.9237% / 1.0579`

### 3. 当前生产决策
- 结构模块代码保留，但生产配置保持 `enabled=false`（避免压低当前基线表现）。
- 当前仍使用上一版稳定基线：
  - 年化 `15.9469%`
  - 最大回撤 `-10.0831%`
  - Sharpe `1.1939`

## 2026-02-22 结构级升级（Phase 2：波动率状态机 + 回撤恢复 + 自适应集中度）

### 1. 新增结构能力
- 在 `structural_upgrade` 下新增 Phase 2 配置并接入：
  - `phase2_vol_*`：波动率状态机（calm/normal/stress）
  - `phase2_recovery_*`：回撤恢复因子（恢复期加速）
  - `phase2_adaptive_*`：自适应集中度（`top_n` 与 `score_power` 动态调整）
- 同步实现于：
  - `/Users/haojc/work/quant-system/scripts/stock_etf/paper_forward_stock.py`
  - `/Users/haojc/work/quant-system/scripts/stock_etf/run_stock_etf.py`

### 2. 搜索验证
- 报告：`/Users/haojc/work/quant-system/outputs/reports/stock_structural_phase2_search_v1.json`
- 搜索规模：`320` 组
- 结果：
  - `年化 > 20%`：`0`
  - `Sharpe > 2`：`0`
  - 双目标命中：`0`
  - 超过当前稳定基线（年化+Sharpe+回撤同时更优）：`0`
  - best_ann：`17.9084% / -24.0808% / 0.9336`
  - best_sh：`13.4051% / -13.8456% / 1.0554`

### 3. 结论
- Phase 2 结构能力已具备并完成初轮验证，但在当前 ETF 池与样本上仍未超过既有稳定基线。
- 生产配置保持：`structural_upgrade.enabled=false`、`phase2_enabled=false`。

## 2026-02-23 目标突破（年化 > 20%，Sharpe > 2）

### 1. 关键改动
- 修复回测与生产不一致：在 `paper_forward_stock.py` 中接入 `risk_overlay`，并支持 `force_rebalance` 语义。
- 生产执行安全修复：`run_stock_etf.py` 的执行守卫在读取历史状态时，自动剔除退池旧标的，避免继续持有无效 symbol。
- 股票池升级为高成长+防守三资产：
  - `159915`（创业板）
  - `513100`（纳指ETF）
  - `518880`（黄金ETF，防守）
...
### 2. 目标搜索结果
- 报告：`/Users/haojc/work/quant-system/outputs/reports/stock_target_search_20_2_local_v2.json`
- 搜索规模：`1000` 组（局部高密度随机搜索）
- 双目标命中（`ann>20%` 且 `Sharpe>2`）：`60` 组
- 最优组合（全样本）：
  - 年化：`34.4308%`
  - 最大回撤：`-13.6494%`
  - Sharpe：`2.1369`
...
### 3. 当前配置回测确认
- 报告：`/Users/haojc/work/quant-system/outputs/reports/backtest_stock_etf_report.json`
