...
from scripts.stock_etf.snapshot_utils import load_history_snapshot, save_history_snapshot
...
def prepare_cpcv_history(runtime, stock, risk, force_refresh_snapshot=False):
    if not force_refresh_snapshot:
        snapshot = load_history_snapshot(runtime, snapshot_name="stock_backtest_snapshot")
        if snapshot is not None:
            snapshot_file, snapshot_meta_file, snapshot_meta, history = snapshot
            return history, snapshot_file, snapshot_meta_file, snapshot_meta, "reuse_existing_snapshot"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        snapshot_file, snapshot_meta_file, snapshot_meta, history = save_history_snapshot(
            history_file=history_file,
            runtime=runtime,
            snapshot_name="stock_backtest_snapshot",
            source="backtest_stock_etf_cpcv",
        )
    return history, snapshot_file, snapshot_meta_file, snapshot_meta, "rebuild_snapshot"
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="rebuild stock_backtest_snapshot from paper-forward instead of reusing existing snapshot",
    )
    ...
    try:
        history, snapshot_file, snapshot_meta_file, snapshot_meta, snapshot_source = prepare_cpcv_history(
            runtime=runtime,
            stock=stock,
            risk=risk,
            force_refresh_snapshot=bool(args.force_refresh_snapshot),
        )
    ...
    report = {
        ...
        "data_fingerprint": snapshot_meta.get("fingerprint", {}),
