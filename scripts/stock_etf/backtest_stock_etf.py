...
def backtest_stock_production(runtime, stock_cfg, risk_cfg):
    with tempfile.TemporaryDirectory() as td:
        ...
        summary, history_file, _latest_file = run_paper_forward(rt, stock_cfg, risk_cfg)
        snapshot_file, snapshot_meta_file, snapshot_meta, _snapshot_df = save_history_snapshot(
            history_file=history_file,
            runtime=runtime,
            snapshot_name="stock_backtest_snapshot",
            source="backtest_stock_etf",
        )
    ...
    out["data_fingerprint"] = snapshot_meta.get("fingerprint", {})
    out["snapshot_file"] = snapshot_file
    out["snapshot_meta_file"] = snapshot_meta_file
    out["snapshot_source"] = str(snapshot_meta.get("source", "unknown"))
    return out


def extract_data_fingerprint(stock_result):
    if not isinstance(stock_result, dict):
        return None
    if stock_result.get("mode") == "global_momentum_both":
        prod = stock_result.get("production")
        if isinstance(prod, dict):
            return prod.get("data_fingerprint")
        return None
...
def main():
    ...
    try:
        ...
        print(f"[backtest-stock-etf] signal error: {e}")
    ...
    try:
        ...
        out_report = os.path.join(out_dir, "backtest_stock_etf_report.json")
        with open(out_report, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)

        out_compare = os.path.join(out_dir, "backtest_stock_etf_compare.json")
        with open(out_compare, "w", encoding="utf-8") as f:
            json.dump(compare_report, f, ensure_ascii=False, indent=2)

        # Compatibility outputs used by existing guards/docs.
        legacy_report = os.path.join(out_dir, "backtest_report.json")
        with open(legacy_report, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)

        legacy_compare = os.path.join(out_dir, "backtest_compare.json")
        with open(legacy_compare, "w", encoding="utf-8") as f:
            json.dump(compare_report, f, ensure_ascii=False, indent=2)
    except OSError as e:
        print(f"[backtest-stock-etf] output error: {e}")
        return EXIT_OUTPUT_ERROR

    print(json.dumps(report, ensure_ascii=False, indent=2))
    print(f"[backtest-stock-etf] report -> {out_report}")
    print(f"[backtest-stock-etf] compare -> {out_compare}")
    print(f"[backtest-stock-etf] legacy report -> {legacy_report}")
    print(f"[backtest-stock-etf] legacy compare -> {legacy_compare}")
    return EXIT_OK
