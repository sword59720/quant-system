...
def load_common_dates(crypto_cfg, date_from=None):
    symbols = crypto_cfg.get("symbols", ["BTC/USDT", "ETH/USDT"])
    sets = []
    for s in symbols:
        fp = os.path.join("data", "crypto", f"{s.replace('/', '_')}.csv")
...
def parse_rolling_ratios(raw):
    if raw is None:
        return [0.55, 0.65, 0.75, 0.85]
    out = []
...
def build_windows(dates, rolling_ratios):
    if len(dates) < 200:
        return None
    out = {
        "cutoff_30pct": dates[int(len(dates) * 0.7)],
        "cutoff_20pct": dates[int(len(dates) * 0.8)],
        "rolling": [],
    }
    for ratio in rolling_ratios:
        idx = int(len(dates) * ratio)
        if idx >= len(dates) - 100:
            continue
        out["rolling"].append({"ratio": float(ratio), "start": dates[idx]})
    return out
...
def objective_score(full, o70, o80, rolling_oos, stress):
    ...
    rolling_anns = [mget(x, "annual_return") for x in rolling_oos]
    rolling_shs = [mget(x, "sharpe") for x in rolling_oos]
    rolling_dds = [mget(x, "max_drawdown") for x in rolling_oos]

    robust_oos_ann = min(o70_ann, o80_ann)
    robust_oos_sh = min(o70_sh, o80_sh)
    robust_roll_ann = min(rolling_anns) if rolling_anns else 0.0
    robust_roll_sh = min(rolling_shs) if rolling_shs else 0.0
    dd_worst = min([full_dd, mget(o70, "max_drawdown"), mget(o80, "max_drawdown"), stress_dd] + rolling_dds)

    reward = (
        0.40 * full_ann
        + 0.18 * robust_oos_ann
        + 0.15 * robust_roll_ann
        + 0.10 * stress_ann
        + 0.10 * full_sh
        + 0.04 * robust_oos_sh
        + 0.03 * robust_roll_sh
...
def pass_metrics(m, ann_min, sharpe_min, dd_min):
    return (
        isinstance(m, dict)
        and mget(m, "annual_return") >= float(ann_min)
        and mget(m, "sharpe") >= float(sharpe_min)
        and mget(m, "max_drawdown") >= float(dd_min)
    )


def hard_constraints_pass(full, o70, o80, rolling_oos, stress, target):
    checks = {
        "full": pass_metrics(
            full,
            target["full"]["annual_return_min"],
            target["full"]["sharpe_min"],
            target["full"]["max_drawdown_min"],
        ),
        "oos_30pct": pass_metrics(
            o70,
            target["oos"]["annual_return_min"],
            target["oos"]["sharpe_min"],
            target["oos"]["max_drawdown_min"],
        ),
        "oos_20pct": pass_metrics(
            o80,
            target["oos"]["annual_return_min"],
            target["oos"]["sharpe_min"],
            target["oos"]["max_drawdown_min"],
        ),
        "stress_mild": pass_metrics(
            stress,
            target["stress_mild"]["annual_return_min"],
            target["stress_mild"]["sharpe_min"],
            target["stress_mild"]["max_drawdown_min"],
        ),
    }
    rolling_ok = bool(rolling_oos)
    rolling_checks = []
    for x in rolling_oos:
        ok = pass_metrics(
            x,
            target["rolling_oos"]["annual_return_min"],
            target["rolling_oos"]["sharpe_min"],
            target["rolling_oos"]["max_drawdown_min"],
        )
        rolling_checks.append(
            {
                "window_from": x.get("window_from"),
                "window_ratio": x.get("window_ratio"),
                "pass": bool(ok),
            }
        )
        rolling_ok = rolling_ok and ok
    checks["rolling_oos_all"] = bool(rolling_ok)
    return bool(all(checks.values())), checks, rolling_checks
...
def main():
    ...
    parser.add_argument("--full-min-ann", type=float, default=None)
    parser.add_argument("--full-min-sharpe", type=float, default=None)
    parser.add_argument("--full-min-dd", type=float, default=None)
    parser.add_argument("--oos-min-ann", type=float, default=0.0)
    parser.add_argument("--oos-min-sharpe", type=float, default=0.0)
    parser.add_argument("--oos-min-dd", type=float, default=-0.12)
    parser.add_argument("--rolling-min-ann", type=float, default=0.0)
    parser.add_argument("--rolling-min-sharpe", type=float, default=0.0)
    parser.add_argument("--rolling-min-dd", type=float, default=-0.12)
    parser.add_argument("--stress-min-ann", type=float, default=0.0)
    parser.add_argument("--stress-min-sharpe", type=float, default=0.0)
    parser.add_argument("--stress-min-dd", type=float, default=-0.12)
    parser.add_argument("--rolling-ratios", type=str, default="0.55,0.65,0.75,0.85")
    ...
    dates = load_common_dates(crypto, date_from=args.date_from)
    windows = build_windows(dates, parse_rolling_ratios(args.rolling_ratios))
    if windows is None:
        raise SystemExit("[crypto-opt-target] no sufficient common date history")
    cut70 = windows["cutoff_30pct"]
    cut80 = windows["cutoff_20pct"]
    rolling_windows = windows["rolling"]

    hard_target = {
        "full": {
            "annual_return_min": float(args.min_ann if args.full_min_ann is None else args.full_min_ann),
            "sharpe_min": float(args.min_sharpe if args.full_min_sharpe is None else args.full_min_sharpe),
            "max_drawdown_min": float(args.max_dd if args.full_min_dd is None else args.full_min_dd),
        },
        "oos": {
            "annual_return_min": float(args.oos_min_ann),
            "sharpe_min": float(args.oos_min_sharpe),
            "max_drawdown_min": float(args.oos_min_dd),
        },
        "rolling_oos": {
            "annual_return_min": float(args.rolling_min_ann),
            "sharpe_min": float(args.rolling_min_sharpe),
            "max_drawdown_min": float(args.rolling_min_dd),
        },
        "stress_mild": {
            "annual_return_min": float(args.stress_min_ann),
            "sharpe_min": float(args.stress_min_sharpe),
            "max_drawdown_min": float(args.stress_min_dd),
        },
    }
    ...
    for i in range(args.trials):
        ...
        for w in rolling_windows:
            wm = run_contract_backtest(runtime, cfg, date_from=w["start"])
            if "error" in wm:
                rolling_error = True
                break
            wm["window_ratio"] = float(w["ratio"])
            wm["window_from"] = str(pd.Timestamp(w["start"]).date())
            rolling_metrics.append(wm)
        if rolling_error:
            continue

        score = objective_score(full, o70, o80, rolling_metrics, stress)
        hit_hard_target = hard_target_pass(full, args.min_ann, args.min_sharpe, args.max_dd)
        hit_hard_multi, hard_checks, rolling_checks = hard_constraints_pass(
            full, o70, o80, rolling_metrics, stress, hard_target
        )
        ...
        records.append(
            {
                ...
                "hard_multi_pass": bool(hit_hard_multi),
                "hard_multi_checks": hard_checks,
                "hard_multi_rolling_checks": rolling_checks,
                "robust_guard_pass": bool(hit_robust_guard),
                "params": params,
                "full": full,
                "oos_30pct": o70,
                "oos_20pct": o80,
                "rolling_oos": rolling_metrics,
                "stress_mild": stress,
            }
        )
        print(
            ...
            f"o80={mget(o80,'annual_return'):.3f} hard_multi={int(hit_hard_multi)}",
            flush=True,
        )

    records = sorted(records, key=lambda x: x["score"], reverse=True)
    hard_pass = [x for x in records if x["hard_target_pass"]]
    hard_multi_pass = [x for x in records if x["hard_multi_pass"]]
    ...
    robust_hard_multi_pass = [x for x in records if x["hard_multi_pass"] and x["robust_guard_pass"]]
    ...
    out = {
        ...
        "rolling_windows": [
            {"ratio": float(x["ratio"]), "window_from": str(pd.Timestamp(x["start"]).date())} for x in rolling_windows
        ],
        "target": {"min_ann": args.min_ann, "min_sharpe": args.min_sharpe, "max_dd": args.max_dd},
        "hard_multi_target": hard_target,
        ...
        "best_hard_multi": hard_multi_pass[0] if hard_multi_pass else None,
        ...
        "best_hard_multi_and_robust": robust_hard_multi_pass[0] if robust_hard_multi_pass else None,
        "hard_target_count": len(hard_pass),
        "hard_multi_count": len(hard_multi_pass),
        "robust_guard_count": len(robust_pass),
        "hard_and_robust_count": len(robust_hard_pass),
        "hard_multi_and_robust_count": len(robust_hard_multi_pass),
        "top10": records[:10],
    }
